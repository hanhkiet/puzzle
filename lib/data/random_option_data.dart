...
class RandomOptionData {
  ...
  TrueFalseModel getComplicatedDivision() {
    ...
    if (isMinus) {
      ...
      question =
          "${n1.toInt()}${space!}${additionSign!}${space!}((${subtractionSign!}${n2.toInt()})${space!}${divisionSign!}${space!}${n3.toInt()})";
    } else {
      doubleAnswer = n1 + (n2 / n3);

      question =
          "${n1.toInt()}${space!}${additionSign!}${space!}(${n2.toInt()}${space!}${divisionSign!}${space!}${n3.toInt()})";
    }
  ...
  TrueFalseModel addModel() {
    ...
    if (!isTrueFalseQuiz) {
      ...
      int helpTag = Random().nextInt(4) + 1;
      int ans;
      ...
      if (question == null || question!.isEmpty) {
        s = "$firstDigit $currentSign $secondDigit = $ans";
      } else {
        s = "${question!} = $ans";
      }

      trueFalseModel = TrueFalseModel(stringAnswer, question: s);
    }
    shuffle(stringList);
  ...
  TrueFalseModel addDoubleModel() {
    ...
    if (!isTrueFalseQuiz) {
      ...
      if (question == null || question!.isEmpty) {
        s = "$firstDigit $currentSign $secondDigit = ${getFormattedString(ans)}";
      } else {
